...
from data_process import DataCollatorForPretraining
...
def main():
    ...
    config.image_vocab_size=8192
    config.wpa_size=2

    model = LayoutLMv3ForPretraining.from_pretrained(
    ...
    def tokenize_and_align_labels(examples, augmentation=False):
        ...
        for batch_index in range(len(tokenized_inputs["input_ids"])):
            ...
            aligned_patch=examples["aligned_patches"][batch_index]
            aligned_patch_token=examples["aligned_patch_tokens"][batch_index]

            previous_word_idx = None
            bbox_inputs = []
            aligned_patch_inputs=[]
            aligned_patch_token_inputs=[]
            word_start_marker=[]
            ...
            for word_idx in word_ids:
                ...
                if word_idx is None:
                    ...
                    aligned_patch_inputs.append(-100)
                    aligned_patch_token_inputs.append(-100)
                    word_start_marker.append(-100)
                # We set the label for the first token of each word.
                elif word_idx != previous_word_idx:
                    bbox_inputs.append(bbox[word_idx])
                    aligned_patch_inputs.append(aligned_patch[word_idx])
                    aligned_patch_token_inputs.append(aligned_patch_token[word_idx])
                    word_start_marker.append(1)
                ...
                # the label_all_tokens flag.
                    ...
                    aligned_patch_inputs.append(aligned_patch[word_idx])
                    aligned_patch_token_inputs.append(aligned_patch_token[word_idx])
                    word_start_marker.append(0)
                previous_word_idx = word_idx

            patch_token=examples['patch_tokens'][batch_index]
            bboxes.append(bbox_inputs)
            aligned_patches.append(aligned_patch_inputs)
            aligned_patch_tokens.append(aligned_patch_token_inputs)
            word_start_markers.append(word_start_marker)
        ...
        tokenized_inputs['aligned_patch']=aligned_patches
        tokenized_inputs['aligned_patch_token'] = aligned_patch_tokens
        tokenized_inputs['word_start_marker'] = word_start_markers
        tokenized_inputs['patch_token'] = patch_tokens
    ...
    def compute_metrics(p):
        ...
        results=glue_compute_metrics(preds=true_predictions, labels=true_labels)
        return results
    ...
    if training_args.do_predict:
        ...
        print(glue_compute_metrics(preds=predictions, labels=labels))
